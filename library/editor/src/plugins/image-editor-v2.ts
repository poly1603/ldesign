...
export class ImageEditorV2 {
  ...
  private injectStyles() {
    if (document.getElementById('image-editor-v2-styles')) return;
  ...
  private selectImage(img: HTMLElement) {
    this.deselectImage();
    this.selectedImage = img;
    this.selectedImage.classList.add('ie-selected');
    this.originalSrc = (img as HTMLImageElement).src;
  ...
  private buildMenu(container: HTMLElement, items: any[]) {
    items.forEach(item => {
      if (item.divider) {
  ...
  private setAlignment(align: string) {
    if (!this.selectedImage) return;
  ...
  private setFloat(floatType: string) {
    if (!this.selectedImage) return;
  ...
  private replaceImage() {
    ...
    if (newSrc) {
      (this.selectedImage as HTMLImageElement).src = newSrc;
  ...
  private addCaption() {
    ...
    if (caption) {
      ...
      if (!figure) {
        ...
        this.selectedImage.parentNode?.insertBefore(figure, this.selectedImage);
