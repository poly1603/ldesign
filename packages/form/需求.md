...
请帮我实现一个功能完整的 Vue 3 表单系统，该系统需要支持 Vue 组件、Composition API hook 以及原生
JavaScript 三种使用方式。系统应该通过配置驱动的方式快速创建功能丰富的表单，并完全支持 Vue 3 的响应式
系统和 v-model 双向绑定。
...
const formOptions = {
  fields: [
    { name: 'username', title: '用户名', component: 'FormInput', required: true },
...
### 2. Composition API Hook 方式

```typescript
import { useForm } from '@ldesign/form'

const { formData, validate, reset, setFieldValue, getFieldValue, errors, renderForm } = useForm({
  fields: [
    { name: 'username', title: '用户名', component: 'FormInput', required: true },
    { name: 'email', title: '邮箱', component: 'FormInput', props: { type: 'email' } },
  ],
...
<html>
  ...
  <body>
    ...
    <script>
      const { createApp } = Vue
      const { DynamicForm, createFormInstance } = LDesignForm
      ...
      // 方式2: 使用原生JavaScript API
      const formInstance = createFormInstance({
        container: '#form-container', // 挂载容器
        options: {
          fields: [
            { name: 'username', title: '用户名', component: 'FormInput', required: true },
            { name: 'email', title: '邮箱', component: 'FormInput', props: { type: 'email' } },
      ...
      // 原生API操作
      formInstance.setFieldValue('username', 'admin')
...
### 4. 原生 JavaScript 模块化使用

```javascript
// ES模块方式
import { createFormInstance, DynamicForm } from '@ldesign/form'
...
// CommonJS方式
const { createFormInstance } = require('@ldesign/form')

// 原生JavaScript类实例化方式
class MyForm {
  constructor(container, options) {
    this.formInstance = createFormInstance({
      container,
      options,
  ...
  // 提供便捷方法
  setData(data) {
    this.formInstance.setFormData(data)
  }

  getData() {
    return this.formInstance.getFormData()
  }
...
// 使用
const myForm = new MyForm('#form-container', {
  fields: [{ name: 'username', title: '用户名', component: 'FormInput', required: true }],
})
```

## 原生 JavaScript 支持的特殊要求

### 1. 构建输出

- **UMD 格式**：支持 script 标签直接引入，全局变量为`LDesignForm`
- **ES 模块**：支持现代模块化项目导入
- **CommonJS**：支持 Node.js 环境使用
- **类型声明**：提供完整的.d.ts 文件支持 TypeScript 项目

### 2. 原生 API 设计

- **createFormInstance**: 创建表单实例的工厂函数
- **FormInstance 类**：提供完整的表单操作 API
...
2. **文件组织**：
   - `src/index.ts`: 主入口文件（Vue 组件和 Hook 导出）
   - `src/vanilla.ts`: 原生 JavaScript 入口文件
   - `src/components/`: 组件目录
   - `src/composables/`: 组合式函数目录
   - `src/types/`: 类型定义目录
   - `src/utils/`: 工具函数目录
   - `src/core/`: 核心逻辑目录（与框架无关的纯逻辑）
3. **构建配置**：
   - 支持 ES 模块、CommonJS 模块和 UMD 格式
   - 分别构建 Vue 版本和原生 JavaScript 版本
   - 提供压缩和未压缩版本
   - 生成完整的类型声明文件
4. **文档示例**：提供详细的使用示例和 API 文档，包含所有三种使用方式的完整示例
5. **测试用例**：编写单元测试确保功能正确性，覆盖 Vue 组件、Hook 和原生 JavaScript 三种使用方式

请按照上述需求设计并实现这个表单系统，确保代码结构清晰、功能完整、易于维护和扩展。特别注意原生
JavaScript 的支持要做到开箱即用，无需额外的框架依赖。同时提供多种使用场景的示例代码以验证功能完整性
。
