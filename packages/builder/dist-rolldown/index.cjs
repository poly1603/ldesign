	...
	FileSystem = class {
		...
		*/
		static async writeFile(filePath, content, encoding = "utf8") {
			await this.ensureDir(path.default.dirname(filePath));
		...
		*/
		static async copyFile(src, dest) {
			await this.ensureDir(path.default.dirname(dest));
		...
		*/
		static async removeFile(filePath) {
			if (await this.exists(filePath)) return fs.promises.unlink(filePath);
		}
		/**
		* 创建目录
		*/
		...
		*/
		static async removeDir(dirPath) {
			if (await this.exists(dirPath)) return fs.promises.rmdir(dirPath, { recursive: true });
		}
		/**
		* 清空目录
		*/
		static async emptyDir(dirPath) {
			if (await this.exists(dirPath)) {
				...
				await Promise.all(files.map(async (file) => {
					...
					const stat$5 = await fs.promises.stat(filePath);
		...
		static async readDirRecursive(dirPath) {
			...
			const traverse = async (currentPath) => {
				...
				for (const item of items) {
					const itemPath = path.default.join(currentPath, item);
					const stat$5 = await fs.promises.stat(itemPath);
		...
		*/
		static async createTempFile(prefix = "temp", suffix = ".tmp") {
			...
			return path.default.join(tempDir, fileName);
		}
		/**
		* 创建临时目录
		*/
		static async createTempDir(prefix = "temp") {
			...
			const tempDirPath = path.default.join(tempDir, dirName);
	...
	*/
	...
	var ReaddirpStream = class ReaddirpStream extends Readable {
		static get defaultOptions() {
			return {
				root: ".",
				fileFilter: (path$9) => true,
				directoryFilter: (path$9) => true,
				type: FILE_TYPE,
				lstat: false,
				depth: 2147483648,
				alwaysStat: false
		...
		async _read(batch) {
			...
			try {
				while (!this.destroyed && batch > 0) {
					const { path: path$9, depth: depth$1, files = [] } = this.parent || {};
					if (files.length > 0) {
						const slice = files.splice(0, batch).map((dirent) => this._formatEntry(dirent, path$9));
		...
		async _exploreDir(path$9, depth$1) {
			let files;
			try {
				files = await readdir$1(path$9, this._rdOptions);
			} catch (error) {
				this._onError(error);
			}
			return {
		...
		async _formatEntry(dirent, path$9) {
			...
			try {
				...
				const fullPath = sysPath$3.resolve(sysPath$3.join(path$9, basename$1));
				entry = {
...
var require_anymatch = /* @__PURE__ */ __commonJS({ "../../node_modules/.pnpm/anymatch@3.1.3/node_modules/anymatch/index.js": ((exports, module) => {
	...
	*/
	const matchPatterns = (patterns, negPatterns, args, returnIndex) => {
		...
		const path$9 = normalizePath$1(_path, false);
...
var require_is_binary_path = /* @__PURE__ */ __commonJS({ "../../node_modules/.pnpm/is-binary-path@2.1.0/node_modules/is-binary-path/index.js": ((exports, module) => {
	...
	module.exports = (filePath) => extensions.has(path$4.extname(filePath).slice(1).toLowerCase());
}) });
	...
	*/
	function createFsWatchInstance(path$9, options, listener, errHandler, emitRaw) {
		const handleEvent = (rawEvent, evPath) => {
			...
			if (evPath && path$9 !== evPath) fsWatchBroadcast(sysPath$2.resolve(path$9, evPath), KEY_LISTENERS, sysPath$2.join(path$9, evPath));
		};
		try {
			return fs$3.watch(path$9, options, handleEvent);
		} catch (error) {
			errHandler(error);
		}
	}
	...
	var NodeFsHandler$1 = class {
		...
		*/
		_watchWithNodeFs(path$9, listener) {
			...
			const directory = sysPath$2.dirname(path$9);
			const basename$1 = sysPath$2.basename(path$9);
			const parent = this.fsw._getWatchedDir(directory);
			parent.add(basename$1);
			const absolutePath = sysPath$2.resolve(path$9);
			...
			const listener = async (path$9, newStats) => {
				...
				if (!newStats || newStats.mtimeMs === 0) try {
					...
					if (isLinux && prevStats.ino !== newStats$1.ino) {
						...
						this.fsw._addPathCloser(path$9, this._watchWithNodeFs(file, listener));
		...
		*/
		async _handleSymlink(entry, directory, path$9, item) {
			...
			if (!this.fsw.options.followSymlinks) {
				...
				let linkPath;
				try {
					linkPath = await fsrealpath(path$9);
				} catch (e) {
					this.fsw._emitReady();
					return true;
				}
				...
				if (dir.has(item)) {
					if (this.fsw._symlinkPaths.get(full) !== linkPath) {
						...
						this.fsw._emit(EV_CHANGE$2, path$9, entry.stats);
					}
				} else {
					dir.add(item);
					this.fsw._symlinkPaths.set(full, linkPath);
					this.fsw._emit(EV_ADD$2, path$9, entry.stats);
				}
				this.fsw._emitReady();
				return true;
			}
		...
		_handleRead(directory, initialAdd, wh, target, dir, depth$1, throttler) {
			...
			let stream = this.fsw._readdirp(directory, {
				...
				let path$9 = sysPath$2.join(directory, item);
				current.add(item);
				...
				if (item === target || !target && !previous.has(item)) {
					this.fsw._incrReadyCount();
					path$9 = sysPath$2.join(dir, sysPath$2.relative(dir, path$9));
		...
		*/
		async _addToNodeFs(path$9, initialAdd, priorWh, depth$1, target) {
			...
			const wh = this.fsw._getWatchHelpers(path$9, depth$1);
			...
			try {
				...
				const follow = this.fsw.options.followSymlinks && !path$9.includes(STAR) && !path$9.includes(BRACE_START$1);
				let closer;
				if (stats.isDirectory()) {
					const absPath = sysPath$2.resolve(path$9);
					const targetPath = follow ? await fsrealpath(path$9) : path$9;
					if (this.fsw.closed) return;
					...
					if (this.fsw.closed) return;
					if (targetPath !== void 0) this.fsw._symlinkPaths.set(sysPath$2.resolve(path$9), targetPath);
				} else closer = this._handleFile(wh.watchPath, stats, initialAdd);
				ready();
				this.fsw._addPathCloser(path$9, closer);
				return false;
			} catch (error) {
...
var require_fsevents_handler = /* @__PURE__ */ __commonJS({ "../../node_modules/.pnpm/chokidar@3.6.0/node_modules/chokidar/lib/fsevents-handler.js": ((exports, module) => {
	...
	*/
	const createFSEventsInstance = (path$9, callback) => {
		const stop = fsevents.watch(path$9, callback);
		return { stop };
	};
	/**
	...
	* @returns {Function} closer
	*/
	function setFSEventsListener(path$9, realPath, listener, rawEmitter) {
		let watchPath = sysPath$1.extname(realPath) ? sysPath$1.dirname(realPath) : realPath;
		const parentPath = sysPath$1.dirname(watchPath);
		let cont = FSEventsWatchers.get(watchPath);
		if (couldConsolidate(parentPath)) watchPath = parentPath;
		const resolvedPath = sysPath$1.resolve(path$9);
	...
	const couldConsolidate = (path$9) => {
		let count = 0;
		for (const watchPath of FSEventsWatchers.keys()) if (watchPath.indexOf(path$9) === 0) {
			count++;
	...
	const calcDepth = (path$9, root) => {
		let i = 0;
		while (!path$9.indexOf(root) && (path$9 = sysPath$1.dirname(path$9)) !== root) i++;
		return i;
	};
	...
	*/
	var FsEventsHandler$1 = class {
		...
		*/
		...
		checkIgnored(path$9, stats) {
			const ipaths = this.fsw._ignoredPaths;
			if (this.fsw._isIgnored(path$9, stats)) {
				ipaths.add(path$9);
				if (stats && stats.isDirectory()) ipaths.add(path$9 + ROOT_GLOBSTAR);
				return true;
			}
			ipaths.delete(path$9);
			ipaths.delete(path$9 + ROOT_GLOBSTAR);
		}
		addOrChange(path$9, fullPath, realPath, parent, watchedDir, item, info, opts) {
			const event = watchedDir.has(item) ? EV_CHANGE$1 : EV_ADD$1;
			this.handleEvent(event, path$9, fullPath, realPath, parent, watchedDir, item, info, opts);
		}
		async checkExists(path$9, fullPath, realPath, parent, watchedDir, item, info, opts) {
			try {
				const stats = await stat$2(path$9);
				if (this.fsw.closed) return;
				if (sameTypes(info, stats)) this.addOrChange(path$9, fullPath, realPath, parent, watchedDir, item, info, opts);
				else this.handleEvent(EV_UNLINK$1, path$9, fullPath, realPath, parent, watchedDir, item, info, opts);
			} catch (error) {
				if (error.code === "EACCES") this.addOrChange(path$9, fullPath, realPath, parent, watchedDir, item, info, opts);
				else this.handleEvent(EV_UNLINK$1, path$9, fullPath, realPath, parent, watchedDir, item, info, opts);
			}
		}
		handleEvent(event, path$9, fullPath, realPath, parent, watchedDir, item, info, opts) {
			if (this.fsw.closed || this.checkIgnored(path$9)) return;
			if (event === EV_UNLINK$1) {
				...
				if (event === EV_ADD$1) {
					if (info.type === FSEVENT_TYPE_DIRECTORY) this.fsw._getWatchedDir(path$9);
					if (info.type === FSEVENT_TYPE_SYMLINK && opts.followSymlinks) {
						const curDepth = opts.depth === void 0 ? void 0 : calcDepth(fullPath, realPath) + 1;
						return this._addToFsEvents(path$9, false, true, curDepth);
					}
				...
				*/
				...
				this.fsw._emit(eventName, path$9);
				if (eventName === EV_ADD_DIR$1) this._addToFsEvents(path$9, false, true);
			}
		}
		/**
		...
		*/
		_watchWithFsEvents(watchPath, realPath, transform$1, globFilter) {
			...
			const watchCallback = async (fullPath, flags, info) => {
				...
				const path$9 = transform$1(sysPath$1.join(watchPath, sysPath$1.relative(watchPath, fullPath)));
				if (globFilter && !globFilter(path$9)) return;
				const parent = sysPath$1.dirname(path$9);
				const item = sysPath$1.basename(path$9);
				const watchedDir = this.fsw._getWatchedDir(info.type === FSEVENT_TYPE_DIRECTORY ? path$9 : parent);
				if (wrongEventFlags.has(flags) || info.event === FSEVENT_UNKNOWN) if (typeof opts.ignored === FUNCTION_TYPE$1) {
					let stats;
					try {
						stats = await stat$2(path$9);
					} catch (error) {}
					if (this.fsw.closed) return;
					if (this.checkIgnored(path$9, stats)) return;
					if (sameTypes(info, stats)) this.addOrChange(path$9, fullPath, realPath, parent, watchedDir, item, info, opts);
					else this.handleEvent(EV_UNLINK$1, path$9, fullPath, realPath, parent, watchedDir, item, info, opts);
				} else this.checkExists(path$9, fullPath, realPath, parent, watchedDir, item, info, opts);
				else switch (info.event) {
					case FSEVENT_CREATED:
					case FSEVENT_MODIFIED: return this.addOrChange(path$9, fullPath, realPath, parent, watchedDir, item, info, opts);
					case FSEVENT_DELETED:
					case FSEVENT_MOVED: return this.checkExists(path$9, fullPath, realPath, parent, watchedDir, item, info, opts);
				}
			};
		...
		*/
		async _handleFsEventsSymlink(linkPath, fullPath, transform$1, curDepth) {
			...
			try {
				...
				this.fsw._incrReadyCount();
				this._addToFsEvents(linkTarget || linkPath, (path$9) => {
					let aliasedPath = linkPath;
					if (linkTarget && linkTarget !== DOT_SLASH) aliasedPath = path$9.replace(linkTarget, linkPath);
					else if (path$9 !== DOT_SLASH) aliasedPath = sysPath$1.join(linkPath, path$9);
		...
		*/
		...
		initWatch(realPath, path$9, wh, processPath) {
			if (this.fsw.closed) return;
		...
		* @returns {Promise<void>}
		*/
		async _addToFsEvents(path$9, transform$1, forceAdd, priorDepth) {
			if (this.fsw.closed) return;
			const opts = this.fsw.options;
			...
			try {
				...
				if (stats.isDirectory()) {
					if (!wh.globFilter) this.emitAdd(processPath(path$9), stats, processPath, opts, forceAdd);
			...
			if (opts.persistent && forceAdd !== true) if (typeof transform$1 === FUNCTION_TYPE$1) this.initWatch(void 0, path$9, wh, processPath);
			else {
				let realPath;
				try {
					realPath = await realpath(wh.watchPath);
				} catch (e) {}
				this.initWatch(realPath, path$9, wh, processPath);
			}
		}
	};
...
var require_chokidar = /* @__PURE__ */ __commonJS({ "../../node_modules/.pnpm/chokidar@3.6.0/node_modules/chokidar/index.js": ((exports) => {
	const { EventEmitter: EventEmitter$5 } = require("events");
	...
	*/
	...
	const normalizeIgnored = (cwd = EMPTY_STR) => (path$9) => {
		if (typeof path$9 !== STRING_TYPE) return path$9;
		return normalizePathToUnix(sysPath.isAbsolute(path$9) ? path$9 : sysPath.join(cwd, path$9));
	};
	const getAbsolutePath = (path$9, cwd) => {
		if (sysPath.isAbsolute(path$9)) return path$9;
		if (path$9.startsWith(BANG)) return BANG + sysPath.join(cwd, path$9.slice(1));
	...
	*/
	...
	var WatchHelper = class {
		constructor(path$9, watchPath, follow, fsw) {
			this.fsw = fsw;
			this.path = path$9 = path$9.replace(REPLACER_RE, EMPTY_STR);
			this.watchPath = watchPath;
			this.fullWatchPath = sysPath.resolve(watchPath);
			this.hasGlob = watchPath !== path$9;
			/** @type {object|boolean} */
			if (path$9 === EMPTY_STR) this.hasGlob = false;
			this.globSymlink = this.hasGlob && follow ? void 0 : false;
			this.globFilter = this.hasGlob ? anymatch(path$9, void 0, ANYMATCH_OPTS) : false;
			this.dirParts = this.getDirParts(path$9);
		...
		getDirParts(path$9) {
			...
			const parts = [];
			const expandedPath = path$9.includes(BRACE_START) ? braces.expand(path$9) : [path$9];
			expandedPath.forEach((path$10) => {
				parts.push(sysPath.relative(this.watchPath, path$10).split(SLASH_OR_BACK_SLASH_RE));
			});
			return parts;
		}
		filterDir(entry) {
	...
	*/
	var FSWatcher = class extends EventEmitter$5 {
		constructor(_opts) {
			super();
			const opts = {};
		...
		*/
		add(paths_, _origAdd, _internal) {
			...
			if (cwd) paths = paths.map((path$9) => {
				...
				if (disableGlobbing || !isGlob(path$9)) return absPath;
				return normalizePath(absPath);
			});
			paths = paths.filter((path$9) => {
				if (path$9.startsWith(BANG)) {
					this._ignoredPaths.add(path$9.slice(1));
					return false;
				}
				this._ignoredPaths.delete(path$9);
				this._ignoredPaths.delete(path$9 + SLASH_GLOBSTAR);
			...
			if (this.options.useFsEvents && this._fsEventsHandler) {
				...
				paths.forEach((path$9) => this._fsEventsHandler._addToFsEvents(path$9));
			} else {
				if (!this._readyCount) this._readyCount = 0;
				this._readyCount += paths.length;
				Promise.all(paths.map(async (path$9) => {
					const res = await this._nodeFsHandler._addToNodeFs(path$9, !_internal, 0, 0, _origAdd);
		...
		*/
		unwatch(paths_) {
			...
			paths.forEach((path$9) => {
				...
				this._ignoredPaths.add(path$9);
				if (this._watched.has(path$9)) this._ignoredPaths.add(path$9 + SLASH_GLOBSTAR);
				this._userIgnored = void 0;
			});
			return this;
		}
		...
		*/
		async _emit(event, path$9, val1, val2, val3) {
			if (this.closed) return;
			const opts = this.options;
			if (isWindows) path$9 = sysPath.normalize(path$9);
			if (opts.cwd) path$9 = sysPath.relative(opts.cwd, path$9);
			/** @type Array<any> */
			const args = [event, path$9];
			...
			if (opts.atomic) {
				if (event === EV_UNLINK) {
					this._pendingUnlinks.set(path$9, args);
					setTimeout(() => {
				...
				if (event === EV_ADD && this._pendingUnlinks.has(path$9)) {
			...
			if (event === EV_CHANGE) {
				const isThrottled = !this._throttle(EV_CHANGE, path$9, 50);
		...
		_awaitWriteFinish(path$9, threshold, event, awfEmit) {
			...
			const awaitWriteFinish = (prevStat) => {
				fs$1.stat(fullPath, (err, curStat) => {
					if (err || !this._pendingWrites.has(path$9)) {
						if (err && err.code !== "ENOENT") awfEmit(err);
						return;
					}
					const now$1 = Number(/* @__PURE__ */ new Date());
					if (prevStat && curStat.size !== prevStat.size) this._pendingWrites.get(path$9).lastChange = now$1;
					const pw = this._pendingWrites.get(path$9);
					const df = now$1 - pw.lastChange;
					if (df >= threshold) {
						this._pendingWrites.delete(path$9);
						awfEmit(void 0, curStat);
					} else timeoutHandler = setTimeout(awaitWriteFinish, this.options.awaitWriteFinish.pollInterval, curStat);
				});
			};
			if (!this._pendingWrites.has(path$9)) {
				this._pendingWrites.set(path$9, {
					lastChange: now,
					cancelWait: () => {
						this._pendingWrites.delete(path$9);
		...
		*/
		_isIgnored(path$9, stats) {
			...
			if (!this._userIgnored) {
				...
				const paths = arrify(ignored).filter((path$10) => typeof path$10 === STRING_TYPE && !isGlob(path$10)).map((path$10) => path$10 + SLASH_GLOBSTAR);
				const list = this._getGlobIgnored().map(normalizeIgnored(cwd)).concat(ignored, paths);
				this._userIgnored = anymatch(list, void 0, ANYMATCH_OPTS);
			}
			return this._userIgnored([path$9, stats]);
		}
		_isntIgnored(path$9, stat$5) {
			return !this._isIgnored(path$9, stat$5);
		}
		/**
		...
		*/
		_getWatchHelpers(path$9, depth$1) {
			const watchPath = depth$1 || this.options.disableGlobbing || !isGlob(path$9) ? path$9 : globParent(path$9);
			const follow = this.options.followSymlinks;
			return new WatchHelper(path$9, watchPath, follow, this);
		}
		/**
		...
		*/
		_remove(directory, item, isDirectory$1) {
			...
			if (!this.options.useFsEvents) this._closePath(path$9);
		}
		/**
...
var ConfigLoader = class {
	...
	*/
	async findConfigFile(startDir = process.cwd()) {
		for (const fileName of CONFIG_FILE_NAMES) {
			const configPath = path.default.resolve(startDir, fileName);
	...
	*/
	async getConfigFileInfo(configPath) {
		...
		const ext = path.default.extname(configPath);
		let type;
		switch (ext) {
			case ".ts":
				type = "ts";
		...
		if (fileExists) try {
			const stats = await import("fs").then((fs$6) => fs$6.promises.stat(configPath));
	...
	*/
	async loadJSConfig(configPath) {
		try {
			const jiti$1 = (0, jiti.default)(require("url").pathToFileURL(__filename).href, {
				interopDefault: true,
				esmResolve: true,
				cache: false
			});
			const configModule = await jiti$1(configPath);
			let config;
	...
	*/
	async loadJSONConfig(configPath) {
		try {
			...
			if (path.default.basename(configPath) === "package.json") {
	...
	*/
	getAllConfigPaths(baseDir = process.cwd()) {
		return CONFIG_FILE_NAMES.map((name) => path.default.resolve(baseDir, name));
	}
	/**
	* 检查配置文件是否存在
	*/
	...
	*/
	getConfigFilePriority(configPath) {
		const fileName = path.default.basename(configPath);
...
var Logger = class Logger {
	...
	*/
	error(message, ...args) {
		if (this.shouldLog(LogLevelEnum.ERROR)) this.log("ERROR", message, chalk.default.red, ...args);
	}
	/**
	* 警告日志
	*/
	warn(message, ...args) {
		if (this.shouldLog(LogLevelEnum.WARN)) this.log("WARN", message, chalk.default.yellow, ...args);
	}
	/**
	* 信息日志
	*/
	info(message, ...args) {
		if (this.shouldLog(LogLevelEnum.INFO)) this.log("INFO", message, chalk.default.blue, ...args);
	}
	/**
	* 调试日志
	*/
	debug(message, ...args) {
		if (this.shouldLog(LogLevelEnum.DEBUG)) this.log("DEBUG", message, chalk.default.gray, ...args);
	}
	/**
	* 详细日志
	*/
	verbose(message, ...args) {
		if (this.shouldLog(LogLevelEnum.VERBOSE)) this.log("VERBOSE", message, chalk.default.gray, ...args);
	}
	/**
	* 成功日志
	*/
	success(message, ...args) {
		if (this.shouldLog(LogLevelEnum.INFO)) this.log("SUCCESS", message, chalk.default.green, ...args);
	}
	/**
	* 开始日志（带缩进）
	*/
	start(message, ...args) {
		if (this.shouldLog(LogLevelEnum.INFO)) this.log("START", `▶ ${message}`, chalk.default.cyan, ...args);
	}
	/**
	* 完成日志（带缩进）
	*/
	complete(message, ...args) {
		if (this.shouldLog(LogLevelEnum.INFO)) this.log("COMPLETE", `✓ ${message}`, chalk.default.green, ...args);
	}
	/**
	* 失败日志（带缩进）
	*/
	fail(message, ...args) {
		if (this.shouldLog(LogLevelEnum.ERROR)) this.log("FAIL", `✗ ${message}`, chalk.default.red, ...args);
	}
	/**
	* 进度日志
	*/
	progress(current, total, message) {
		if (this.shouldLog(LogLevelEnum.INFO)) {
			...
			this.log("PROGRESS", `${progressBar} ${percent}%${progressMessage}`, chalk.default.cyan);
		}
	}
	/**
	* 表格日志
	...
	*/
	group(label) {
		if (this.shouldLog(LogLevelEnum.INFO)) console.group(this.formatMessage("GROUP", label, chalk.default.cyan));
	}
	/**
	* 分组结束
	*/
	...
	*/
	divider(char = "-", length = 50) {
		if (this.shouldLog(LogLevelEnum.INFO)) console.log(chalk.default.gray(char.repeat(length)));
	}
	/**
	* 创建子日志记录器
	*/
	...
	*/
	formatMessage(type, message, colorFn) {
		...
		if (this.timestamp) {
			...
			formatted += chalk.default.gray(`[${timestamp}] `);
		}
		if (this.prefix) formatted += chalk.default.gray(`[${this.prefix}] `);
	...
	*/
	createProgressBar(percent, width = 20) {
		...
		return this.colors ? chalk.default.cyan(bar) : bar;
	}
};
/**
* 默认日志记录器实例
...
* 提供配置文件的完整生命周期管理
*/
var ConfigManager = class extends events.EventEmitter {
	logger;
	errorHandler;
	options;
	currentConfig;
...
* 插件管理器类
*/
var PluginManager = class extends events.EventEmitter {
	logger;
	options;
	...
	async detectConfigFiles(projectPath, scores, evidence) {
		for (const [type, pattern] of Object.entries(LIBRARY_TYPE_PATTERNS)) {
			...
			for (const configFile of pattern.configs) {
				const configPath = path.default.join(projectPath, configFile);
...
* 性能监控器类
*/
var PerformanceMonitor = class extends events.EventEmitter {
	logger;
	options;
...
var TestRunner = class {
	...
	*/
	async detectFramework(projectRoot) {
		...
		for (const [framework, config] of Object.entries(TEST_FRAMEWORKS)) for (const configFile of config.configFiles) if (await fs_extra.pathExists(path.join(projectRoot, configFile))) return framework;
	...
	*/
	async detectPackageManager(projectRoot) {
		if (await fs_extra.pathExists(path.join(projectRoot, "pnpm-lock.yaml"))) return "pnpm";
		if (await fs_extra.pathExists(path.join(projectRoot, "yarn.lock"))) return "yarn";
		if (await fs_extra.pathExists(path.join(projectRoot, "package-lock.json"))) return "npm";
		return "npm";
	}
	/**
	* 执行测试命令
	*/
	async executeTests(cwd, runCommand, testCommand, args, timeout) {
		return new Promise((resolve$1, reject) => {
			...
			const child = (0, child_process.spawn)(runCommand, fullCommand, {
				cwd,
				stdio: "pipe",
				shell: true
			});
...
var ValidationReporter = class {
	...
	*/
	async outputJsonReport(report, config) {
		...
		await fs_extra.ensureDir(path.dirname(outputPath));
		await fs_extra.writeFile(outputPath, reportJson, "utf8");
	...
	*/
	async outputHtmlReport(report, config) {
		...
		await fs_extra.ensureDir(path.dirname(outputPath));
		await fs_extra.writeFile(outputPath, html, "utf8");
	...
	*/
	async outputMarkdownReport(report, config) {
		...
		await fs_extra.ensureDir(path.dirname(outputPath));
		await fs_extra.writeFile(outputPath, markdown, "utf8");
...
var TemporaryEnvironment = class {
	...
	async copyBuildOutputs(context) {
		...
		try {
			for (const output of context.buildResult.outputs) {
				...
				if (await fs_extra.pathExists(sourcePath)) {
					await fs_extra.ensureDir(path.dirname(targetPath));
					await fs_extra.copy(sourcePath, targetPath);
	...
	async cleanup(context) {
		...
		try {
			if (context.tempDir && this.tempDirs.has(context.tempDir)) {
				await fs_extra.remove(context.tempDir);
	...
	*/
	async createTempDirectory(context) {
		const tempDirName = `ldesign-builder-validation-${(0, crypto.randomUUID)().slice(0, 8)}`;
		const baseTempDir = context.config.environment?.tempDir ? path.resolve(context.projectRoot, context.config.environment.tempDir) : path.join(os.tmpdir(), "ldesign-builder");
	...
	*/
	async copySourceFiles(context) {
		...
		if (await fs_extra.pathExists(srcDir)) {
			const targetSrcDir = path.join(context.tempDir, "src");
			await fs_extra.copy(srcDir, targetSrcDir);
	...
	*/
	async updatePackageJson(context) {
		...
		try {
			...
			await fs_extra.writeJson(packageJsonPath, packageJson, { spaces: 2 });
	...
	*/
	async exists(tempDir) {
		return fs_extra.pathExists(tempDir);
	}
	/**
	* 获取临时目录大小
	*/
	...
	*/
	async getDirectoryStats(dirPath) {
		let totalSize = 0;
		let totalFiles = 0;
		const items = await fs_extra.readdir(dirPath);
		for (const item of items) {
			const itemPath = path.join(dirPath, item);
			const stats = await fs_extra.stat(itemPath);
			if (stats.isDirectory()) {
...
* 打包后验证器实现
*/
var PostBuildValidator = class extends events.EventEmitter {
	/** 验证配置 */
	config;
	/** 测试运行器 */
	testRunner;
	...
	*/
	async validate(context) {
		const validationId = (0, crypto.randomUUID)();
		const startTime = Date.now();
...
var RollupAdapter = class {
	...
	*/
	async createUMDConfig(config) {
		...
		const fs$6 = await import("fs");
		const path$9 = await import("path");
		...
		for (const entry of possibleEntries) if (fs$6.existsSync(path$9.resolve(process.cwd(), entry))) {
			umdEntry = entry;
			break;
		}
		let umdName = umdConfig.name || outputConfig.name;
		if (!umdName) try {
			const packageJson = JSON.parse(fs$6.readFileSync(path$9.resolve(process.cwd(), "package.json"), "utf-8"));
	...
	*/
	generateBuildInfo(buildInfoConfig) {
		...
		if (config.version !== false) try {
			...
			const packageJson = JSON.parse(fs$6.readFileSync("package.json", "utf-8"));
			parts.push(`Version: ${packageJson.version}`);
		} catch {}
		if (config.buildTime !== false) parts.push(`Built: ${(/* @__PURE__ */ new Date()).toISOString()}`);
		if (config.environment !== false) parts.push(`Environment: ${process.env.NODE_ENV || "development"}`);
...
var RolldownAdapter = class {
	...
	*/
	loadRolldown() {
		try {
			if (typeof require !== "undefined") return require("rolldown");
...
*/
var LibraryBuilder = class extends events.EventEmitter {
	/** 当前状态 */
...
var PathUtils = class {
	...
	*/
	static normalize(filePath) {
		return path.default.posix.normalize(filePath.replace(/\\/g, "/"));
	}
	/**
	* 解析绝对路径
	*/
	...
	*/
	static relative(from, to) {
		return this.normalize(path.default.relative(from, to));
	}
	/**
	* 连接路径
	*/
	static join(...paths) {
		return this.normalize(path.default.join(...paths));
	}
	/**
	* 获取目录名
	*/
	static dirname(filePath) {
		return this.normalize(path.default.dirname(filePath));
	}
	/**
	* 获取文件名（包含扩展名）
	*/
	...
	*/
	static extname(filePath) {
		return path.default.extname(filePath);
	}
	/**
	* 获取文件名（不包含扩展名）
	*/
	...
	*/
	static parse(filePath) {
		const parsed = path.default.parse(filePath);
		return {
			root: parsed.root,
	...
	*/
	static format(pathObject) {
		return this.normalize(path.default.format(pathObject));
	}
	/**
	* 检查路径是否在指定目录内
	*/
	...
	*/
	static getCommonParent(path1, path2) {
		...
		const parts1 = abs1.split(path.default.sep);
		const parts2 = abs2.split(path.default.sep);
		const commonParts = [];
		const minLength = Math.min(parts1.length, parts2.length);
		for (let i = 0; i < minLength; i++) if (parts1[i] === parts2[i]) commonParts.push(parts1[i]);
		else break;
		return commonParts.join(path.default.sep) || path.default.sep;
	}
	/**
	* 获取路径深度
	*/
	...
	*/
	static fromFileURL(fileURL) {
		return this.normalize((0, url.fileURLToPath)(fileURL));
	}
	/**
	* 获取项目根目录
	*/
	static findProjectRoot(startPath) {
		...
		while (currentPath !== path.default.dirname(currentPath)) {
			const packageJsonPath = this.join(currentPath, "package.json");
			if (require("fs").existsSync(packageJsonPath)) return currentPath;
			const gitPath = this.join(currentPath, ".git");
			if (require("fs").existsSync(gitPath)) return currentPath;
			currentPath = path.default.dirname(currentPath);
		}
	...
	*/
	static get sep() {
		return path.default.sep;
	}
	/**
	* 获取平台特定的路径定界符
	*/
	static get delimiter() {
		return path.default.delimiter;
	}
};
...
exports.BuilderStatus = BuilderStatus;
exports.BundlerAdapterFactory = BundlerAdapterFactory;
exports.BundlerFeature = BundlerFeature;
exports.BundlerStatus = BundlerStatus;
...
exports.PluginPhase = PluginPhase;
