...
const banner_generator_1 = require("../../utils/banner-generator");
...
class RollupAdapter {
    constructor(options = {}) {
        ...
        try {
            ...
            this.version = rollup.VERSION || 'unknown';
            this.available = true;
        }
        catch (error) {
            this.version = 'unknown';
            this.available = false;
            this.logger.warn('Rollup 不可用', error);
        }
    ...
    async build(config) {
        if (!this.available) {
            throw new error_handler_1.BuilderError(errors_1.ErrorCode.ADAPTER_NOT_AVAILABLE, 'Rollup 适配器不可用');
        }
        try {
            // 检查是否为清理模式
            const isCleanMode = config?.clean === true;
            this.logger.info(`清理模式检查: config.clean=${config?.clean}, isCleanMode=${isCleanMode}`);
            // 受控启用构建缓存（清理模式下禁用缓存）
            const cacheEnabled = !isCleanMode && this.isCacheEnabled(config);
            const cacheOptions = this.resolveCacheOptions(config);
            const cache = new cache_1.RollupCache({
            ...
            if (cacheEnabled && cachedResult) {
                ...
                this.logger.info('使用缓存的构建结果 (cache hit)');
                return cachedResult;
            }
            ...
            const { gzipSize } = await Promise.resolve().then(() => __importStar(require('gzip-size')));
            const outputs = [];
            let totalRaw = 0;
            ...
            throw new error_handler_1.BuilderError(errors_1.ErrorCode.BUILD_FAILED, `Rollup 构建失败: ${error.message}`, { cause: error });
        }
    }
    /**
     * 启动监听模式
    ...
    async watch(config) {
        ...
        try {
            ...
            throw new error_handler_1.BuilderError(errors_1.ErrorCode.BUILD_FAILED, `启动 Rollup 监听模式失败: ${error.message}`, { cause: error });
        }
    }
    /**
     * 转换配置
        ...
        if (config.output) {
            ...
            if (outputConfig.esm || outputConfig.cjs || outputConfig.umd) {
                ...
                if (outputConfig.esm && outputConfig.esm !== false) {
                    ...
                    const esmInput = esmConfig.input
                        ? await (0, glob_1.normalizeInput)(esmConfig.input, process.cwd())
                        : config.input;
                    // 统一注入 Banner/Footer/Intro/Outro
                    const bannerCfgEsm = config.banner;
                ...
                if (outputConfig.cjs && outputConfig.cjs !== false) {
                    ...
                    const cjsInput = cjsConfig.input
                        ? await (0, glob_1.normalizeInput)(cjsConfig.input, process.cwd())
                        : config.input;
                    // 统一注入 Banner/Footer/Intro/Outro
                    const bannerCfgCjs = config.banner;
                ...
                if (outputConfig.umd || config.umd) {
            ...
            if (Array.isArray(outputConfig.format)) {
                ...
                if (isMultiEntry) {
                    ...
                    const hasUmdSection = Boolean(config.umd || config.output?.umd);
                    if (formats.includes('umd') || config.umd?.enabled || hasUmdSection) {
                        umdConfig = await this.createUMDConfig(config);
                    }
                ...
                for (const format of formats) {
                    ...
                    try {
                        const names = [...(formatPlugins || [])].map((p) => p?.name || '(anon)');
                        this.logger.info(`[${format}] 有效插件: ${names.join(', ')}`);
                    }
                    catch { }
                    const bannerCfg = config.banner;
                    ...
                    configs.push({
                        ...
                        output: {
                            ...
                            entryFileNames,
                            chunkFileNames,
                            exports: isESM ? outputConfig.exports ?? 'auto' : 'named',
                ...
                try {
                    const names = [...(userPlugins || [])].map((p) => p?.name || '(anon)');
                    this.logger.info(`[${format}] 有效插件: ${names.join(', ')}`);
                }
                catch { }
                const bannerCfg2 = config.banner;
     ...
     */
    async transformPluginsForFormat(plugins, outputDir, options) {
        const { emitDts = true } = options || {};
        const transformedPlugins = [];
        for (const plugin of plugins) {
            try {
                const pluginName = (plugin && (plugin.name || plugin?.rollup?.name)) || '';
                ...
                if (plugin.plugin && typeof plugin.plugin === 'function') {
                    ...
                    if (nameLc === 'typescript') {
                        ...
                        const typescript = await Promise.resolve().then(() => __importStar(require('@rollup/plugin-typescript')));
                        // 直接从插件包装对象读取原始选项（在策略中附加）
                        const originalOptions = plugin.options || {};
                        // 清理不被 @rollup/plugin-typescript 支持的字段
                        const { tsconfigOverride: _ignored, compilerOptions: origCO = {}, ...rest } = originalOptions;
                        ...
                        try {
                            ...
                            if (typeof rest.tsconfig === 'string') {
                                const tsconfigAbs = pathMod.resolve(process.cwd(), rest.tsconfig);
                                if (!fsMod.existsSync(tsconfigAbs)) {
                                    delete rest.tsconfig;
                                }
                            }
                        }
                        catch { }
                ...
                // 直接使用已实例化的插件
                    ...
                    // UMD/IIFE 禁止 d.ts 相关插件
                    const inameLc = String(plugin?.name || '').toLowerCase();
     ...
     * 分发类型文件到输出目录
     */
    async distributeTypes() {
        // 暂时不实现，避免TypeScript警告
        return Promise.resolve();
    }
    /**
     * 分发类型文件到输出目录的实际实现
     */
    async _distributeTypesImpl() {
        const typesDir = 'types';
        try {
            // 检查types目录是否存在
            const typesExists = await fsExtra.pathExists(typesDir);
            if (!typesExists) {
                return;
            }
            // 复制到es和lib目录
            ...
            for (const dir of outputDirs) {
                const targetDir = path_1.default.join(dir, 'types');
                await fsExtra.copy(typesDir, targetDir, {
                    filter: (src) => {
                        const stat = fs_1.default.lstatSync(src);
                        if (stat.isDirectory()) {
                            return true;
                        }
            ...
            // 静默处理错误，不影响构建流程
            this.logger.warn('类型文件分发失败', error);
        }
    }
    /**
     * 判断是否启用构建缓存
     ...
     */
    resolveCacheOptions(config) {
        const c = config?.cache;
        const opts = {};
        if (typeof c === 'object' && c) {
            if (typeof c.dir === 'string' && c.dir.trim()) {
                opts.cacheDir = path_1.default.isAbsolute(c.dir) ? c.dir : path_1.default.resolve(process.cwd(), c.dir);
            }
    ...
    async getAcornPlugins() {
        const plugins = [];
        try {
            const jsx = (await Promise.resolve().then(() => __importStar(require('acorn-jsx')))).default;
        ...
        try {
            const ts = (await Promise.resolve().then(() => __importStar(require('acorn-typescript')))).default;
            plugins.push(ts());
        }
        catch (e) {
            // 忽略
    ...
    async loadRollup() {
        try {
            ...
            return await Promise.resolve().then(() => __importStar(require('rollup')));
        }
        catch (error) {
    ...
    async getBasePlugins(config) {
        try {
            const { nodeResolve } = await Promise.resolve().then(() => __importStar(require('@rollup/plugin-node-resolve')));
            const commonjs = (await Promise.resolve().then(() => __importStar(require('@rollup/plugin-commonjs')))).default;
            const json = (await Promise.resolve().then(() => __importStar(require('@rollup/plugin-json')))).default;
    ...
    async getBabelPlugin(config) {
        ...
        try {
            const { getBabelOutputPlugin } = await Promise.resolve().then(() => __importStar(require('@rollup/plugin-babel')));
            const babelOptions = {
    ...
    async createUMDConfig(config) {
        // 处理 boolean 配置
        let umdSection = config.umd || config.output?.umd || {};
        if (umdSection === true) {
            umdSection = {}; // 使用默认配置
        }
        ...
        const projectRoot = config.root || config.cwd || process.cwd();
        ...
        if (umdEntry && (umdEntry.includes('*') || Array.isArray(umdEntry))) {
            const resolved = await (0, glob_1.normalizeInput)(umdEntry, projectRoot);
            // UMD 必须是单入口
     ...
     * 解析 Banner
     */
    async resolveBanner(bannerConfig, config) {
        // 显式禁用
        if (bannerConfig === false)
            return undefined;
        const banners = [];
        // 自定义 Banner
        ...
        try {
            const projectInfo = await banner_generator_1.BannerGenerator.getProjectInfo();
            const auto = banner_generator_1.BannerGenerator.generate({
                bundler: this.name,
                bundlerVersion: this.version !== 'unknown' ? this.version : undefined,
                ...projectInfo,
                buildMode: config?.mode || process.env.NODE_ENV || 'production',
                minified: Boolean(config?.minify)
            });
            return auto;
        }
        catch {
    ...
    async resolveFooter(bannerConfig) {
        ...
        // 默认 Footer（未提供时自动生成）
        try {
            const info = await banner_generator_1.BannerGenerator.getProjectInfo();
            if (info.projectName) {
