...
exports.PostBuildValidator = void 0;
const events_1 = require("events");
const crypto_1 = require("crypto");
const TestRunner_1 = require("./TestRunner");
const ValidationReporter_1 = require("./ValidationReporter");
const TemporaryEnvironment_1 = require("./TemporaryEnvironment");
 ...
 * 打包后验证器实现
 */
class PostBuildValidator extends events_1.EventEmitter {
    /**
     * 构造函数
     */
    constructor(config = {}, options = {}) {
        ...
        // 初始化依赖
        this.logger = options.logger || new logger_1.Logger({ level: 'info', prefix: 'PostBuildValidator' });
        this.errorHandler = options.errorHandler || new error_handler_1.ErrorHandler({ logger: this.logger });
        // 初始化组件
        this.testRunner = new TestRunner_1.TestRunner({
            logger: this.logger,
            errorHandler: this.errorHandler
        });
        this.reporter = new ValidationReporter_1.ValidationReporter({
            logger: this.logger
        });
        this.tempEnvironment = new TemporaryEnvironment_1.TemporaryEnvironment({
            logger: this.logger,
            errorHandler: this.errorHandler
        });
    ...
    async validate(context) {
        const validationId = (0, crypto_1.randomUUID)();
        ...
        try {
            ...
            const validationError = this.errorHandler.createError(errors_1.ErrorCode.BUILD_FAILED, '验证过程失败', { cause: error });
            // 执行验证失败钩子
