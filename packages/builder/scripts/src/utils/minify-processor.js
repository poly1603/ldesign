...
exports.MinifyProcessor = void 0;
exports.createMinifyProcessor = createMinifyProcessor;
exports.getDefaultMinifyConfig = getDefaultMinifyConfig;
...
class MinifyProcessor {
     ...
     * 创建 Rollup 压缩插件
     */
    createRollupMinifyPlugin(options) {
        ...
        try {
            ...
            this.logger.warn('压缩插件加载失败，将跳过压缩:', error.message);
        }
        return plugins;
    }
    /**
     * 创建 Rolldown 压缩配置
     */
    createRolldownMinifyConfig(options) {
        const config = this.resolveMinifyConfig(options);
        if (!config.js) {
            return { minify: false };
        }
    ...
    createCSSMinifyPlugin(options) {
        ...
        try {
            ...
            this.logger.warn('CSS 压缩插件加载失败:', error.message);
            return null;
        }
    }
    /**
...
exports.MinifyProcessor = MinifyProcessor;
/**
 * 创建压缩处理器实例
 */
function createMinifyProcessor(logger) {
    return new MinifyProcessor(logger);
}
/**
 * 获取默认压缩配置
 */
function getDefaultMinifyConfig() {
    return {
        level: 'basic',
        js: {
            minifier: 'terser',
