...
exports.BuildPerformanceAnalyzer = void 0;
...
class BuildPerformanceAnalyzer {
    ...
    endPhase(phase) {
        ...
        // 获取性能指标
        const sessionMetrics = this.performanceMonitor.endSession(`phase-${phase}`);
        const memUsage = process.memoryUsage();
        // 更新内存使用信息
    ...
    finishAnalysis(options = {}) {
        ...
        const totalDuration = this.currentAnalysis.phases.reduce((sum, phase) => sum + phase.metrics.duration, 0);
        // 检测全局瓶颈
     ...
     * 检测全局瓶颈
     */
    detectGlobalBottlenecks(phases, options) {
        const issues = [];
        ...
        phases.forEach(phase => {
            ...
            if (phase.metrics.duration > thresholds.slowPhase) {
                bottlenecks.push({
                    type: 'cpu',
                    severity: phase.metrics.duration > thresholds.slowPhase * 2 ? 'high' : 'medium',
                    description: `阶段 ${phase.phase} 执行时间过长`,
                    impact: `影响整体构建速度，占用 ${((phase.metrics.duration / phases.reduce((sum, p) => sum + p.metrics.duration, 0)) * 100).toFixed(1)}% 的构建时间`,
                    suggestion: this.getSuggestionForSlowPhase(phase.phase),
                    phase: phase.phase,
                    metrics: {
                        value: phase.metrics.duration,
                        threshold: thresholds.slowPhase,
                        unit: 'ms'
                    }
                });
            }
            ...
            if (phase.metrics.memoryUsage.peak > thresholds.highMemory) {
                bottlenecks.push({
                    type: 'memory',
                    severity: phase.metrics.memoryUsage.peak > thresholds.highMemory * 2 ? 'critical' : 'high',
                    description: `阶段 ${phase.phase} 内存使用过高`,
                    impact: `可能导致系统内存不足，影响构建稳定性`,
                    suggestion: this.getSuggestionForHighMemory(phase.phase),
                    phase: phase.phase,
                    metrics: {
                        value: phase.metrics.memoryUsage.peak,
    ...
    generateRecommendations(phases, issues) {
        ...
        const slowestPhase = phases.reduce((slowest, phase) => phase.duration > slowest.duration ? phase : slowest);
        ...
        // 缓存相关建议
        const totalCacheOps = phases.reduce((sum, phase) => sum + phase.metrics.cacheHits + phase.metrics.cacheMisses, 0);
     ...
     * 获取慢阶段的建议
     */
    getSuggestionForSlowPhase(phase) {
        const suggestions = {
     ...
     * 获取高内存使用的建议
     */
    getSuggestionForHighMemory(phase) {
        const suggestions = {
...
exports.BuildPerformanceAnalyzer = BuildPerformanceAnalyzer;
