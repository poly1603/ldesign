...
class FileSystem {
    ...
    static async writeFile(filePath, content, encoding = 'utf8') {
        // 确保目录存在
        await this.ensureDir(path__default.dirname(filePath));
    ...
    static async copyFile(src, dest) {
        // 确保目标目录存在
        await this.ensureDir(path__default.dirname(dest));
    ...
    static async removeDir(dirPath) {
        if (await this.exists(dirPath)) {
            return promises.rmdir(dirPath, { recursive: true });
        }
    }
    /**
     * 清空目录
    ...
    static async readDirRecursive(dirPath) {
        ...
        const traverse = async (currentPath) => {
            ...
            for (const item of items) {
                const itemPath = path__default.join(currentPath, item);
    ...
    static async createTempFile(prefix = 'temp', suffix = '.tmp') {
        ...
        return path__default.join(tempDir, fileName);
    }
    /**
     * 创建临时目录
     */
    static async createTempDir(prefix = 'temp') {
        ...
        const tempDirPath = path__default.join(tempDir, dirName);
...
class ConfigLoader {
    ...
    async findConfigFile(startDir = process.cwd()) {
        for (const fileName of CONFIG_FILE_NAMES) {
            const configPath = path__default.resolve(startDir, fileName);
    ...
    async getConfigFileInfo(configPath) {
        ...
        const ext = path__default.extname(configPath);
        let type;
        switch (ext) {
            case '.ts':
                type = 'ts';
    ...
    async loadJSONConfig(configPath) {
        try {
            ...
            // 特殊处理 package.json
            if (path__default.basename(configPath) === 'package.json') {
    ...
    getAllConfigPaths(baseDir = process.cwd()) {
        return CONFIG_FILE_NAMES.map(name => path__default.resolve(baseDir, name));
    }
    /**
     * 检查配置文件是否存在
     */
    ...
    getConfigFilePriority(configPath) {
        const fileName = path__default.basename(configPath);
 ...
 * 提供配置文件的完整生命周期管理
 */
class ConfigManager extends EventEmitter$1 {
    constructor(options = {}) {
        super();
        this.options = {
            validateOnLoad: true,
    ...
    async loadConfig(options = {}) {
        try {
            ...
            if (options.validate !== false && this.options.validateOnLoad) {
                ...
                if (!validation.valid) {
                    throw new BuilderError(ErrorCode.CONFIG_VALIDATION_ERROR, `配置验证失败: ${validation.errors.join(', ')}`);
                }
            }
            // 冻结配置（如果启用）
 ...
 * 插件管理器类
 */
class PluginManager extends EventEmitter$1 {
    constructor(options = {}) {
        super();
    ...
    async loadPlugin(plugin) {
        ...
        try {
            ...
            if (this.options.blacklist?.includes(plugin.name)) {
                throw new BuilderError(ErrorCode.PLUGIN_LOAD_ERROR, `插件 ${plugin.name} 在黑名单中`);
            }
            // 检查白名单
            if (this.options.whitelist?.length && !this.options.whitelist.includes(plugin.name)) {
                throw new BuilderError(ErrorCode.PLUGIN_LOAD_ERROR, `插件 ${plugin.name} 不在白名单中`);
            }
            // 检查插件数量限制
            if (this.plugins.size >= (this.options.maxPlugins || 100)) {
                throw new BuilderError(ErrorCode.PLUGIN_LOAD_ERROR, `插件数量超过限制 (${this.options.maxPlugins})`);
            }
            // 验证插件
    ...
    async loadPlugins(plugins) {
        ...
        for (const plugin of plugins) {
            ...
            if (!result.success && !plugin.enabled) {
                throw new BuilderError(ErrorCode.PLUGIN_LOAD_ERROR, `必需插件 ${plugin.name} 加载失败`, { cause: result.error });
            }
        }
        return results;
    }
    ...
    getPerformanceStats(name) {
        if (name) {
            ...
            if (!stats) {
                throw new BuilderError(ErrorCode.PLUGIN_NOT_FOUND, `插件 ${name} 不存在`);
            }
            return stats;
        }
    ...
    validatePlugin(plugin) {
        if (!plugin.name) {
            throw new BuilderError(ErrorCode.PLUGIN_LOAD_ERROR, '插件必须有名称');
        }
        if (typeof plugin.name !== 'string') {
            throw new BuilderError(ErrorCode.PLUGIN_LOAD_ERROR, '插件名称必须是字符串');
        }
        // 检查插件依赖
        if (plugin.dependencies) {
            for (const dep of plugin.dependencies) {
                if (!this.plugins.has(dep)) {
                    throw new BuilderError(ErrorCode.PLUGIN_DEPENDENCY_ERROR, `插件 ${plugin.name} 依赖的插件 ${dep} 未找到`);
                }
            }
        }
    }
    ...
    async detectConfigFiles(projectPath, scores, evidence) {
        for (const [type, pattern] of Object.entries(LIBRARY_TYPE_PATTERNS)) {
            ...
            for (const configFile of pattern.configs) {
                const configPath = path__default.join(projectPath, configFile);
                if (await exists(configPath)) {
 ...
 * 性能监控器类
 */
class PerformanceMonitor extends EventEmitter$1 {
    constructor(options = {}) {
        super();
...
class TestRunner {
    ...
    async detectFramework(projectRoot) {
        ...
        const packageJsonPath = path$3.join(projectRoot, 'package.json');
        if (await fs$4.pathExists(packageJsonPath)) {
            const packageJson = await fs$4.readJson(packageJsonPath);
            const allDeps = {
                ...packageJson.dependencies,
                ...packageJson.devDependencies
            };
        ...
        for (const [framework, config] of Object.entries(TEST_FRAMEWORKS)) {
            for (const configFile of config.configFiles) {
                if (await fs$4.pathExists(path$3.join(projectRoot, configFile))) {
                    return framework;
                }
            }
        }
    ...
    async detectPackageManager(projectRoot) {
        // 检查锁文件
        if (await fs$4.pathExists(path$3.join(projectRoot, 'pnpm-lock.yaml'))) {
            return 'pnpm';
        }
        if (await fs$4.pathExists(path$3.join(projectRoot, 'yarn.lock'))) {
            return 'yarn';
        }
        if (await fs$4.pathExists(path$3.join(projectRoot, 'package-lock.json'))) {
            return 'npm';
        }
        // 默认使用 npm
        return 'npm';
...
class ValidationReporter {
    ...
    async outputJsonReport(report, config) {
        ...
        await fs$4.ensureDir(path$3.dirname(outputPath));
        await fs$4.writeFile(outputPath, reportJson, 'utf8');
    ...
    async outputHtmlReport(report, config) {
        ...
        await fs$4.ensureDir(path$3.dirname(outputPath));
        await fs$4.writeFile(outputPath, html, 'utf8');
    ...
    async outputMarkdownReport(report, config) {
        ...
        await fs$4.ensureDir(path$3.dirname(outputPath));
        await fs$4.writeFile(outputPath, markdown, 'utf8');
...
class TemporaryEnvironment {
    ...
    async copyBuildOutputs(context) {
        ...
        try {
            ...
            for (const output of context.buildResult.outputs) {
                // 假设输出文件在输出目录中
                const sourcePath = path$3.join(context.outputDir, output.fileName);
                const targetPath = path$3.join(context.tempDir, output.fileName);
                if (await fs$4.pathExists(sourcePath)) {
                    await fs$4.ensureDir(path$3.dirname(targetPath));
                    await fs$4.copy(sourcePath, targetPath);
    ...
    async cleanup(context) {
        ...
        try {
            if (context.tempDir && this.tempDirs.has(context.tempDir)) {
                await fs$4.remove(context.tempDir);
    ...
    async createTempDirectory(context) {
        ...
        const baseTempDir = context.config.environment?.tempDir
            ? path$3.resolve(context.projectRoot, context.config.environment.tempDir)
            : path$3.join(os.tmpdir(), 'ldesign-builder');
        const tempDir = path$3.join(baseTempDir, tempDirName);
        await fs$4.ensureDir(tempDir);
    ...
    async copyProjectFiles(context) {
        ...
        for (const file of filesToCopy) {
            const sourcePath = path$3.join(context.projectRoot, file);
            if (await fs$4.pathExists(sourcePath)) {
                const targetPath = path$3.join(context.tempDir, file);
                await fs$4.copy(sourcePath, targetPath);
    ...
    async copyTestFiles(context) {
        ...
        for (const pattern of testPatterns) {
            ...
            for (const testFile of testFiles) {
                const sourcePath = path$3.join(context.projectRoot, testFile);
                const targetPath = path$3.join(context.tempDir, testFile);
                await fs$4.ensureDir(path$3.dirname(targetPath));
                await fs$4.copy(sourcePath, targetPath);
    ...
    async copySourceFiles(context) {
        ...
        const srcDir = path$3.join(context.projectRoot, 'src');
        if (await fs$4.pathExists(srcDir)) {
            const targetSrcDir = path$3.join(context.tempDir, 'src');
            await fs$4.copy(srcDir, targetSrcDir);
    ...
    async updatePackageJson(context) {
        const packageJsonPath = path$3.join(context.tempDir, 'package.json');
        if (!(await fs$4.pathExists(packageJsonPath))) {
        ...
        try {
            ...
            // 保存更新后的 package.json
            await fs$4.writeJson(packageJsonPath, packageJson, { spaces: 2 });
    ...
    async exists(tempDir) {
        return fs$4.pathExists(tempDir);
    }
    /**
     * 获取临时目录大小
     */
    ...
    async getDirectoryStats(dirPath) {
        let totalSize = 0;
        let totalFiles = 0;
        const items = await fs$4.readdir(dirPath);
        for (const item of items) {
            const itemPath = path$3.join(dirPath, item);
            const stats = await fs$4.stat(itemPath);
 ...
 * 打包后验证器实现
 */
class PostBuildValidator extends EventEmitter$1 {
    /**
     * 构造函数
     */
...
class RolldownAdapter {
    ...
    async build(config) {
        if (!this.available) {
            throw new BuilderError(ErrorCode.ADAPTER_NOT_AVAILABLE, 'Rolldown 适配器不可用');
        }
        try {
 ...
 */
class LibraryBuilder extends EventEmitter$1 {
    constructor(options = {}) {
        super();
        /** 当前状态 */
...
class PathUtils {
    ...
    static normalize(filePath) {
        return path__default.posix.normalize(filePath.replace(/\\/g, '/'));
    }
    /**
     * 解析绝对路径
     */
    static resolve(...paths) {
        return path__default.resolve(...paths);
    }
    /**
     * 获取相对路径
     */
    static relative(from, to) {
        return this.normalize(path__default.relative(from, to));
    }
    /**
     * 连接路径
     */
    static join(...paths) {
        return this.normalize(path__default.join(...paths));
    }
    /**
     * 获取目录名
     */
    static dirname(filePath) {
        return this.normalize(path__default.dirname(filePath));
    }
    /**
     * 获取文件名（包含扩展名）
     */
    static basename(filePath, ext) {
        return path__default.basename(filePath, ext);
    }
    /**
     * 获取文件扩展名
     */
    ...
    static isAbsolute(filePath) {
        return path__default.isAbsolute(filePath);
    }
    /**
     * 转换为绝对路径
     */
    ...
    static parse(filePath) {
        const parsed = path__default.parse(filePath);
        return {
            root: parsed.root,
    ...
    static format(pathObject) {
        return this.normalize(path__default.format(pathObject));
    }
    /**
     * 检查路径是否在指定目录内
     */
    ...
    static getCommonParent(path1, path2) {
        ...
        const parts1 = abs1.split(path__default.sep);
        const parts2 = abs2.split(path__default.sep);
        const commonParts = [];
        ...
        return commonParts.join(path__default.sep) || path__default.sep;
    }
    /**
     * 获取路径深度
     */
    ...
    static findProjectRoot(startPath) {
        ...
        while (currentPath !== path__default.dirname(currentPath)) {
            // 检查是否存在 package.json
            ...
            currentPath = path__default.dirname(currentPath);
        }
        // 如果没找到，返回起始路径
     ...
     */
    static get sep() {
        return path__default.sep;
    }
    /**
     * 获取平台特定的路径定界符
     */
    static get delimiter() {
        return path__default.delimiter;
    }
}
// 导出便捷函数
...
var isWin32 = os__default.platform() === 'win32';
