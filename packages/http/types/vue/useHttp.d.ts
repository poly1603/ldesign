...
import { UseRequestReturn, UseQueryReturn, UseMutationReturn } from '@/types/vue';
...
declare function useHttp(): {
    ...
    request: <T = any>(config: RequestConfig) => Promise<__types.ResponseData<T>>;
    get: <T = any>(url: string, config?: RequestConfig) => Promise<__types.ResponseData<T>>;
    post: <T = any>(url: string, data?: any, config?: RequestConfig) => Promise<__types.ResponseData<T>>;
    put: <T = any>(url: string, data?: any, config?: RequestConfig) => Promise<__types.ResponseData<T>>;
    delete: <T = any>(url: string, config?: RequestConfig) => Promise<__types.ResponseData<T>>;
    patch: <T = any>(url: string, data?: any, config?: RequestConfig) => Promise<__types.ResponseData<T>>;
    head: <T = any>(url: string, config?: RequestConfig) => Promise<__types.ResponseData<T>>;
    options: <T = any>(url: string, config?: RequestConfig) => Promise<__types.ResponseData<T>>;
...
declare function useResource<T = any>(baseUrl: string): {
    ...
    list: (params?: Record<string, any>) => Promise<__types.ResponseData<T[]>>;
    detail: (id: string | number) => Promise<__types.ResponseData<T>>;
    create: (data: T) => Promise<__types.ResponseData<T>>;
    update: (id: string | number, data: Partial<T>) => Promise<__types.ResponseData<T>>;
    patch: (id: string | number, data: Partial<T>) => Promise<__types.ResponseData<T>>;
    remove: (id: string | number) => Promise<__types.ResponseData<any>>;
};
/**
 * 分页查询 Hook
 */
declare function usePagination<T = any>(baseUrl: string, initialPage?: number, initialPageSize?: number): {
    ...
    invalidate: () => void;
    execute: (config?: RequestConfig) => Promise<__types.ResponseData<{
        data: T[];
        total: number;
        page: number;
        pageSize: number;
    }>>;
    refresh: () => Promise<__types.ResponseData<{
        data: T[];
        total: number;
        page: number;
        pageSize: number;
    ...
    data: Ref<T_1 | null>;
    loading: Ref<boolean>;
    error: Ref<__types.HttpError | null>;
