 ...
 * 严格类型的 HTTP 客户端接口
 */
interface TypedHttpClient<TBaseResponse = any> extends HttpClient {
 ...
 * API 端点配置
 */
interface ApiEndpoint<TResponse = any, TRequest = any> {
    url: string;
    method: HttpMethod;
 ...
 * 类型化的请求配置
 */
interface TypedRequestConfig<TData = any> extends Omit<RequestConfig, 'data'> {
    data?: TData;
}
/**
 * 类型化的响应数据
 */
interface TypedResponseData<TData = any> extends Omit<ResponseData, 'data'> {
    data: TData;
}
/**
 * 状态码类型
 */
type HttpStatusCode = 200 | 201 | 202 | 204 | 400 | 401 | 403 | 404 | 409 | 422 | 429 | 500 | 502 | 503 | 504;
/**
 * 内容类型枚举
 */
...
export { ContentType, RequestPriority };
export type { ApiEndpoint, CacheConfig, CacheStorage, ConcurrencyConfig, ErrorInterceptor, ExtendedRequestConfig, HttpAdapter, HttpClient, HttpClientConfig, HttpError, HttpMethod, HttpStatusCode, InterceptorManager, RequestConfig, RequestInterceptor, ResponseData, ResponseInterceptor, RetryConfig, TypedHttpClient, TypedRequestConfig, TypedResponseData };
