 ...
 * useRequest 配置选项
 */
interface UseRequestOptions<T = any> extends RequestConfig {
    /** 是否立即执行请求 */
    immediate?: boolean;
    /** 初始数据 */
    initialData?: T;
 ...
 * useRequest 返回值
 */
interface UseRequestReturn<T = any> extends RequestState<T> {
    /** 执行请求 */
 ...
 * useQuery 配置选项
 */
interface UseQueryOptions<T = any> extends UseRequestOptions<T> {
    /** 查询键 */
 ...
 * useQuery 返回值
 */
interface UseQueryReturn<T = any> extends UseRequestReturn<T> {
    /** 是否来自缓存 */
    isStale: Ref<boolean>;
    /** 是否正在后台更新 */
 ...
 * useMutation 配置选项
 */
interface UseMutationOptions<T = any, V = any> {
    /** 成功回调 */
 ...
 * useMutation 返回值
 */
interface UseMutationReturn<T = any, V = any> extends RequestState<T> {
    /** 执行变更 */
...
export { HttpClient, HttpError, RequestConfig, ResponseData };
export type { HttpPluginOptions, RequestState, UseMutationOptions, UseMutationReturn, UseQueryOptions, UseQueryReturn, UseRequestOptions, UseRequestReturn };
