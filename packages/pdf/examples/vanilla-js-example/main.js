...
function calculateSpecialZoom(mode) {
  if (!AppState.pdfDocument) return 1.0;
...
async function handleZoomSelectChange(event) {
  ...
  const scale = isNaN(value) ? value : parseFloat(value);
  await setZoom(scale);
}
...
async function performSearch(query) {
  if (!AppState.pdfDocument || !query) return;
...
function highlightSearchResult(index) {
  if (index < 0 || index >= AppState.searchResults.length) return;
...
async function generateThumbnails() {
  if (!AppState.pdfDocument || !Elements.thumbnailsContainer) return;
...
async function loadOutline() {
  if (!AppState.pdfDocument || !Elements.outlineContainer) return;
...
async function loadAttachments() {
  if (!AppState.pdfDocument || !Elements.attachmentsContainer) return;
...
function updateZoomDisplay() {
  if (Elements.zoomSelect) {
    ...
    for (const option of Elements.zoomSelect.options) {
      if (parseFloat(option.value) === currentScale) {
        option.selected = true;
        found = true;
        break;
      }
...
function downloadPDF() {
  if (!AppState.pdfDocument || !AppState.fileName) return;
