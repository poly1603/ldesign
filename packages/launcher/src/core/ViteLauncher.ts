...
import { ErrorHandler, ProjectDetector, ConfigManager, PluginManager } from '@/services';
...
export class ViteLauncher implements IViteLauncher {
      ...
      if (exists && !options.force) {
        ...
        if (files.length > 0) {
          throw ErrorHandler.createError(
            ...
            `目录 ${absolutePath} 不为空。使用 --force 选项覆盖。`
          );
        }
      }
   ...
   * @returns 项目信息
   */
  async getProjectInfo(projectPath: string = process.cwd()) {
    try {
      const absolutePath = path.resolve(projectPath);
      const detection = await this.projectDetector.detectProjectType(absolutePath);
      ...
      return {
        ...
        isViteProject: detection.report.detectedFiles.some((file: string) => file.includes('vite.config')),
        framework: detection.framework,
        hasTypeScript: detection.report.detectedFiles.some((file: string) => file.endsWith('.ts') || file.endsWith('.tsx')),
        packageManager: 'npm', // 默认值，可以根据需要检测
        dependencies: detection.report.dependencies
      };
    } catch (error) {
      const launcherError = this.errorHandler.handleError(
        error as Error,
...
<html lang="en">
  ...
  private async generateEntryFiles(srcDir: string, projectType: ProjectType): Promise<void> {
    switch (projectType) {
      case 'vue3':
        ...
        break;
      case 'react':
        await fs.writeFile(path.join(srcDir, 'main.jsx'),
  ...
  private shouldLog(level: LogLevel): boolean {
    const levels: LogLevel[] = ['silent', 'error', 'warn', 'info'];
