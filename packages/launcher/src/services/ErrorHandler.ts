...
export class ErrorHandler implements IErrorHandler {
  ...
  private initializeErrorSuggestions(): void {
    this.errorSuggestions.set(ERROR_CODES.PROJECT_TYPE_DETECTION_FAILED, 
      '请确保项目根目录包含 package.json 文件，并检查依赖配置是否正确');
    
    this.errorSuggestions.set(ERROR_CODES.INVALID_CONFIG, 
      '请检查 Vite 配置文件语法是否正确，参考官方文档进行配置');
    
    this.errorSuggestions.set(ERROR_CODES.PLUGIN_LOAD_FAILED, 
      '请确保插件已正确安装，检查插件版本兼容性');
    
    this.errorSuggestions.set(ERROR_CODES.BUILD_FAILED, 
      '请检查代码语法错误，确保所有依赖已正确安装');
    
    this.errorSuggestions.set(ERROR_CODES.DEV_SERVER_START_FAILED, 
      '请检查端口是否被占用，或尝试使用其他端口号');
    
    this.errorSuggestions.set(ERROR_CODES.PREVIEW_SERVER_START_FAILED, 
      '请先执行构建命令，确保构建产物存在');
    
    this.errorSuggestions.set(ERROR_CODES.CONFIG_MERGE_FAILED, 
      '请检查配置对象结构是否正确，避免循环引用');
    
    this.errorSuggestions.set(ERROR_CODES.DEPENDENCY_NOT_FOUND, 
      '请运行 npm install 或 yarn install 安装缺失的依赖');
    
    this.errorSuggestions.set(ERROR_CODES.UNSUPPORTED_FRAMEWORK, 
      '当前框架暂不支持，请使用 Vue2/Vue3/React/Vanilla/Lit 中的一种');
    
    this.errorSuggestions.set(ERROR_CODES.INVALID_PROJECT_ROOT, 
      '请确保项目根目录路径正确且可访问');
  }
