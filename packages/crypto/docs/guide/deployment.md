...
// 在加密操作中使用
export const monitoredEncrypt = (data: string, key: string) => {
  try {
...
// 监控加密性能
export const monitoredEncryptPerf = (data: string, key: string) => {
...
class SecurityAudit {
  ...
  private static async auditKeyStrength(issues: string[], recommendations: string[]) {
    ...
    if (lastRotation) {
      const rotationAge = Date.now() - parseInt(lastRotation)
