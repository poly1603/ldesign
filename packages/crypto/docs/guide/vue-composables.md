...
const { encryptRSA, decryptRSA, generateRSAKeyPair, isEncrypting, isDecrypting } = useCrypto()
...
const { encodeBase64, decodeBase64, encodeHex, decodeHex } = useCrypto()
...
const { hmacMd5, hmacSha1, hmacSha256, hmacSha384, hmacSha512, verifyHmac } = useHash()
...
const { sign, verify, isSigning, isVerifying, lastError, clearError } = useSignature()
