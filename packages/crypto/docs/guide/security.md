...
class KeyRotationManager {
  ...
  generateNewKey(): string {
    const newKeyId = (parseInt(this.currentKeyId) + 1).toString()
  ...
  cleanupOldKeys(keepVersions = 3): void {
    const currentVersion = parseInt(this.currentKeyId)
    const keysToDelete = []
    
    for (const [keyId] of this.keys) {
      const version = parseInt(keyId)
...
// 在加密操作中使用
const auditedEncrypt = (data: string, key: string) => {
  try {
