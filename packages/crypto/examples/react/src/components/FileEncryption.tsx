...
export const FileEncryption: React.FC = () => {
  ...
  // 解密文件
  const decryptFile = useCallback((encryptedFile: EncryptedFile) => {
    if (!password) {
      alert('请输入解密密码')
      return
    }
  ...
  return (
    <div className="file-encryption">
            ...
            {encryptedFiles.map((file, index) => (
              <div key={index} className="file-card">
                ...
                <div className="file-info">
                  <p><strong>原始大小:</strong> {formatFileSize(file.originalSize)}</p>
                  <p><strong>加密时间:</strong> {file.timestamp.toLocaleString()}</p>
                  <p><strong>文件哈希:</strong> 
      ...
      <div className="security-info">
        <h4>🔒 安全特性:</h4>
        <ul>
          <li><strong>AES-256-CBC 加密:</strong> 军用级别的加密标准</li>
          <li><strong>随机 IV:</strong> 每次加密使用不同的初始化向量</li>
          <li><strong>完整性验证:</strong> SHA-256 哈希确保文件未被篡改</li>
          <li><strong>密码保护:</strong> 只有正确密码才能解密文件</li>
        </ul>
      </div>
    </div>
  )
