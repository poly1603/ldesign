...
import { aes, des, hash, base64, hex } from '@ldesign/crypto'
...
export const PerformanceBenchmark: React.FC = () => {
  ...
  const runFullBenchmark = useCallback(async () => {
    ...
    const tests = [
      ...
      // AES 解密测试
      ...testSizes.map((size, index) => {
        const encrypted = aes.encrypt(testData[index], password, { keySize: 256 })
        return {
          name: `AES-256 解密 (${formatDataSize(size)})`,
          test: () => runBenchmark(
            'AES-256',
            '解密',
            () => encrypted.success && encrypted.data ? aes.decrypt(encrypted.data, password, { keySize: 256 }) : null,
            size
          )
        }
      }),
          ...
          <div className="performance-summary">
            ...
            <div className="summary-grid">
              <div className="summary-item">
                <h5>最快操作</h5>
                <p>{results.reduce((fastest, current) => 
                  current.avgTime < fastest.avgTime ? current : fastest
                ).algorithm} {results.reduce((fastest, current) => 
                  current.avgTime < fastest.avgTime ? current : fastest
                ).operation}</p>
              ...
              <div className="summary-item">
                <h5>最慢操作</h5>
                <p>{results.reduce((slowest, current) => 
                  current.avgTime > slowest.avgTime ? current : slowest
                ).algorithm} {results.reduce((slowest, current) => 
                  current.avgTime > slowest.avgTime ? current : slowest
                ).operation}</p>
                <span className="time-value">
              ...
              <div className="summary-item">
                <h5>平均执行时间</h5>
                <span className="time-value">
                  {(results.reduce((sum, result) => sum + result.avgTime, 0) / results.length).toFixed(2)} ms
                </span>
              </div>
            </div>
          </div>
