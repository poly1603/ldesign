...
export class EncryptedStorage {
  ...
  async store(
    ...
    // Generate unique ID
    const id = this.csprng.randomUUID()
  ...
  private async encryptData(
    ...
    const actualKeyId = keyId || await this.keyManager.generateKey({
      name: 'storage-key',
      algorithm: 'AES',
      purpose: 'encryption',
      keySize: 256
    ...
    switch (this.options.encryptionMode) {
      case EncryptionMode.AES_256_GCM: {
        ...
        const cipher = crypto.createCipheriv('aes-256-gcm', Buffer.from(keyMaterial, 'hex'), iv)
      ...
      case EncryptionMode.CHACHA20_POLY1305: {
        ...
        keyBytes.set(keyBuffer.slice(0, 32))
        const nonce = this.csprng.randomBytes(12)

        const result = chacha.encrypt(data, keyBytes, nonce)
        const encrypted = new Uint8Array(result.ciphertext.length + result.tag.length)
        encrypted.set(result.ciphertext)
        encrypted.set(result.tag, result.ciphertext.length)
      ...
      case EncryptionMode.AES_256_CBC: {
        const iv = this.csprng.randomBytes(16)
        const cipher = crypto.createCipheriv('aes-256-cbc', Buffer.from(keyMaterial, 'hex'), iv)
  ...
  private async decryptData(
    ...
    // Get decryption key
    const keyMaterial = await this.keyManager.getKey(metadata.keyId!)
    if (!keyMaterial) {
    ...
    switch (metadata.encryptionMode) {
      case EncryptionMode.AES_256_GCM: {
        ...
        const decipher = crypto.createDecipheriv('aes-256-gcm', Buffer.from(keyMaterial, 'hex'), iv)
      ...
      case EncryptionMode.CHACHA20_POLY1305: {
        ...
        const tagLength = 16 // Poly1305 tag is 16 bytes
        const ciphertext = encrypted.slice(0, -tagLength)
        const tag = encrypted.slice(-tagLength)

        const decrypted = chacha.decrypt(ciphertext, tag, keyBytes, iv)
        if (!decrypted) {
          throw new Error('ChaCha20-Poly1305 decryption failed')
        }
        return decrypted
      }
