...
- **系统接口**: 内置登录、用户信息、菜单、权限等系统接口
- **用户认证**: 完整的登录、登出、会话管理功能
- **权限控制**: 基于角色和权限的访问控制
- **模拟数据**: 开发阶段使用模拟数据，便于前端开发
- **状态管理**: 响应式的用户状态和认证状态管理
- **API 演示**: 完整的 API 功能演示页面
- **错误处理**: 统一的 API 错误处理和用户反馈
...
2. **访问应用**
   ...
   - 访问 API 演示页面：http://localhost:3001/api-demo
...
- **🔐 登录系统**: 现代化登录界面，支持真实 API 登录
- **🏠 首页展示**: 应用概览和功能介绍，集成 API 演示入口
- **🚀 API 演示**: 完整的 API 功能演示页面
  - 系统接口调用（登录、用户信息、菜单、权限等）
  - 自定义 API 注册和调用
  - API 调用历史记录
  - 实时状态显示
- **👤 用户状态**: 响应式的用户认证状态管理
