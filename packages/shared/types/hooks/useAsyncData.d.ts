 ...
 * 异步数据获取函数
 */
type AsyncDataFetcher<T, P extends any[] = any[]> = (...params: P) => Promise<T>;
/**
 * 异步数据配置
 */
interface AsyncDataConfig<T, P extends any[] = any[]> {
    /** 默认数据 */
    defaultValue?: T;
    /** 是否立即执行 */
    immediate?: boolean;
 ...
 * ```
 */
declare function useAsyncData<T, P extends any[] = any[]>(fetcher: AsyncDataFetcher<T, P>, config?: AsyncDataConfig<T, P>): {
 ...
 * 获取缓存信息
 */
declare const getCacheInfo: () => {
    size: number;
    keys: string[];
};

export { clearAllCache, clearCache, getCacheInfo, useAsyncData };
export type { AsyncDataConfig, AsyncDataFetcher, AsyncDataState };
