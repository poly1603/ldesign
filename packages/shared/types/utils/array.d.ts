 ...
 * ```
 */
declare function groupBy<T, K extends keyof T>(array: T[], key: K | ((item: T) => string | number)): Record<string, T[]>;
/**
 * 扁平化嵌套数组
 *
 * @param array - 要扁平化的数组
 ...
 * ```
 */
declare function flatten<T>(array: any[], depth?: number): T[];
/**
 * 深度扁平化数组
 *
 * @param array - 要扁平化的数组
 ...
 * ```
 */
declare function flattenDeep<T>(array: any[]): T[];
/**
 * 数组交集
 *
 ...
 * ```
 */
declare function difference<T>(array: T[], ...excludeArrays: T[][]): T[];
/**
 * 随机打乱数组
 *
 * @param array - 要打乱的数组
...
export { average, chunk, compact, difference, flatten, flattenDeep, groupBy, intersection, max, min, sample, shuffle, sum, union, unique, uniqueBy };
