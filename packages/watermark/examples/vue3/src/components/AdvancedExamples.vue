  ...
  <div class="advanced-examples">
    ...
    <div class="grid grid-2">
      ...
      <div class="card glass">
        ...
        <div class="security-controls">
          ...
          <div class="form-group">
            <label>
              <input v-model="securityConfig.mutationObserver" type="checkbox">
              DOM 变化监控
            </label>
          </div>
          <div class="form-group">
        ...
        <div class="demo-container" ref="securityRef">
        ...
        <div class="controls">
          <button class="btn btn-primary" @click="createSecurityWatermark">创建安全水印</button>
          <button class="btn btn-danger" @click="destroySecurityWatermark">销毁水印</button>
        </div>
        <div class="security-log" v-if="securityLogs.length">
          <h4>安全日志</h4>
          <div class="log-entries">
            <div v-for="log in securityLogs.slice(-5)" :key="log.id" class="log-entry">
      ...
      <div class="card glass">
        ...
        <div class="demo-container" ref="responsiveRef" :style="responsiveContainerStyle">
          <div class="demo-content">
            <p>响应式水印示例</p>
            <p>调整浏览器窗口大小查看效果</p>
            <p>水印会根据容器大小自动调整</p>
        ...
        <div class="controls">
          <button class="btn btn-primary" @click="createResponsiveWatermark">创建响应式水印</button>
          <button class="btn btn-secondary" @click="toggleContainerSize">切换容器大小</button>
        </div>
      </div>
      ...
      <div class="card glass">
        ...
        <div class="animation-controls">
          ...
          <div class="form-group">
            ...
            <input v-model="animationConfig.duration" type="range" min="500" max="5000" step="100">
          </div>
          <div class="form-group">
            <label>缓动函数</label>
        ...
        <div class="demo-container" ref="animationRef">
          <div class="demo-content">
            <p>动画水印示例</p>
            <p>选择不同的动画效果</p>
          </div>
        ...
        <div class="controls">
          <button class="btn btn-primary" @click="createAnimationWatermark">应用动画</button>
          <button class="btn btn-secondary" @click="pauseAnimation">暂停/恢复</button>
        </div>
      </div>
      ...
      <div class="card glass">
        ...
        <div class="render-mode-tabs">
          <button
            ...
            :key="mode.value"
            :class="['mode-tab', { active: currentRenderMode === mode.value }]"
        ...
        <div class="demo-container" ref="renderModeRef">
          <div class="demo-content">
            <p>多渲染模式示例</p>
            <p>切换不同的渲染模式查看效果</p>
          </div>
    ...
    <div class="card glass mt-30">
      ...
      <div class="batch-controls">
        <div class="form-group">
          <label>批量数量</label>
          <input v-model.number="batchConfig.count" type="number" min="1" max="10">
        </div>
        <div class="form-group">
          <label>基础文字</label>
        ...
        <div class="controls">
          <button class="btn btn-primary" @click="createBatchWatermarks">批量创建</button>
          <button class="btn btn-secondary" @click="updateBatchWatermarks">批量更新</button>
          <button class="btn btn-danger" @click="destroyBatchWatermarks">批量销毁</button>
        </div>
      </div>
...
const setBatchRef = (el: any, index: number) => {
...
// 批量管理器
const batchManager = useWatermarkManager()
...
const animationConfig = reactive({
  ...
  duration: 2000,
  easing: 'ease-in-out' as 'ease' | 'ease-in' | 'ease-out' | 'ease-in-out' | 'linear'
})
...
const createSecurityWatermark = async () => {
...
const destroySecurityWatermark = async () => {
  if (securityInstance.value) {
    await destroyWatermark(securityInstance.value)
...
const createResponsiveWatermark = async () => {
...
const createAnimationWatermark = async () => {
...
const switchRenderMode = async (mode: 'dom' | 'canvas' | 'svg') => {
...
const createBatchWatermarks = async () => {
  const startTime = performance.now()
  ...
  try {
    ...
    batchStats.avgCreateTime = Math.round(batchStats.totalCreateTime / batchConfig.count)
...
const updateBatchWatermarks = async () => {
  const promises = []
