...
export class WatermarkError extends Error {
  ...
  getUserFriendlyMessage(): string {
    const messages: Partial<Record<WatermarkErrorCode, string>> = {
      ...
      [WatermarkErrorCode.INVALID_CONTAINER]: '容器元素无效，请提供有效的DOM元素',
      [WatermarkErrorCode.RENDER_FAILED]: '水印渲染失败，请检查配置和环境',
      [WatermarkErrorCode.CANVAS_NOT_SUPPORTED]: '浏览器不支持Canvas，请使用其他渲染模式',
      [WatermarkErrorCode.SVG_NOT_SUPPORTED]: '浏览器不支持SVG，请使用其他渲染模式',
      ...
      [WatermarkErrorCode.BROWSER_NOT_SUPPORTED]: '浏览器版本过低，不支持此功能',
...
// 默认错误配置
export const DEFAULT_ERROR_CONFIG: Required<Omit<ErrorConfig, 'reportUrl' | 'customHandlers' | 'customRecoveryStrategies'>> = {
  enabled: true,
  autoRecover: true,
  reportErrors: false,
  maxErrorHistory: 50,
