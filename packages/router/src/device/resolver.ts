...
export class DeviceComponentResolver {
   ...
   * 创建模板组件
   */
  private createTemplateComponent(record: RouteRecordNormalized): RouteComponent {
    // 返回一个异步组件，延迟加载模板
    return async () => {
      try {
        // 动态导入模板解析器
   ...
   * 检查是否有任何设备组件可用作回退
   */
  private hasAnyDeviceComponent(deviceComponents: Record<string, RouteComponent>): boolean {
