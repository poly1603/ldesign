...
export class DeviceRouterPlugin {
  ...
  constructor(router: Router, options: DeviceRouterPluginOptions = {}) {
    ...
    if (this.options.enableTemplateRoutes) {
      this.templateResolver = new TemplateRouteResolver(this.options.templateConfig)
    }
  }
   ...
   * 标准化选项
   */
  private normalizeOptions(options: DeviceRouterPluginOptions): Required<DeviceRouterPluginOptions> {
    return {
 ...
 * 创建设备路由插件的便捷函数
 */
export function createDeviceRouterPlugin(
