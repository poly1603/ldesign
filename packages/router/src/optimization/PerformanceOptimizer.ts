...
export class SmartPrefetchManager {
  ...
  private shouldPrefetch(path: string): boolean {
    // 已经预取过的不再预取
    if (this.prefetchedRoutes.has(path)) return false;
    
    // 慢速网络下限制预取
    if (this.bandwidth === 'slow' && this.prefetchQueue.size > 2) return false;
    
    // 当前路由不预取
    if (path === this.router.currentRoute.value.path) return false;
  ...
  async prefetch(path: string, priority: 'high' | 'low' = 'low'): Promise<void> {
    if (!this.shouldPrefetch(path)) return;
...
export class RenderingOptimizer {
  ...
  progressiveEnhance(component: any): any {
    if (!this.config.progressiveEnhancement) return component;
