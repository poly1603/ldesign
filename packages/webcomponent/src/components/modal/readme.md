...
| `sheetCloseThreshold`  | `sheet-close-threshold`  | 关闭阈值：低于该高度则关闭，默认 '30%'                                 | `number \| string`                                                                                            | `undefined`    |
| `sheetDraggable`       | `sheet-draggable`        | Bottom Sheet 拖拽开关（仅在 variant='bottom-sheet' 时生效）       | `boolean`                                                                                                     | `true`         |
| `sheetInitial`         | `sheet-initial`          | 初始高度：同上；若不传且有 snapPoints，则使用最大 snap 值（通常是 100%）        | `number \| string`                                                                                            | `undefined`    |
| `sheetSnapPoints`      | `sheet-snap-points`      | Snap 点：数组，值支持像素（数字或'120px'），百分比（'50%'），或小数（0.5 表示 50%） | `(string \| number)[]`                                                                                        | `undefined`    |
