...
| `allowDrag`           | `allow-drag`            | 自定义：是否允许拖拽该节点（JS 赋值）                                              | `(node: TreeNode) => boolean`                                                                                     | `undefined` |
| `allowDrop`           | `allow-drop`            | 自定义：是否允许在目标位置放置（JS 赋值）                                            | `(dragNode: TreeNode, dropNode: TreeNode, position: "after" \| "before" \| "inside") => boolean`                  | `undefined` |
...
| `draggable`           | `draggable`             | 是否开启节点拖拽重排                                                        | `boolean`                                                                                                         | `false`     |
...
| `loadData`            | `load-data`             | 可通过 JS 赋值：自定义加载函数                                                 | `(node?: TreeNode) => Promise<TreeNode[]>`                                                                        | `undefined` |
...
| `transform`           | `transform`             | 可通过 JS 赋值：转换函数，将接口原始数据转换为 TreeNode[]                              | `(raw: any) => TreeNode[]`                                                                                        | `undefined` |
