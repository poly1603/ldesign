...
| -------------------- | ---------------------- | ------------------- | -------------------------------------------------- | ---------------- |
| `anchorAlign`        | `anchor-align`         | 锚点对齐方式              | `"auto" \| "center" \| "end" \| "start"`           | `'start'`        |
| `anchorAutoUpdate`   | `anchor-auto-update`   | 自动更新位置              | `boolean`                                          | `true`           |
| `anchorBoundary`     | `anchor-boundary`      | 边界限制                | `HTMLElement \| string`                            | `'viewport'`     |
| `anchorConstrain`    | `anchor-constrain`     | 约束在边界内              | `boolean`                                          | `true`           |
| `anchorElement`      | `anchor-element`       | 锚点元素（选择器或元素）        | `HTMLElement \| string`                            | `undefined`      |
| `anchorFlip`         | `anchor-flip`          | 自动翻转位置              | `boolean`                                          | `true`           |
| `anchorFollowScroll` | `anchor-follow-scroll` | 跟随滚动                | `boolean`                                          | `true`           |
| `anchorMaskPartial`  | `anchor-mask-partial`  | 部分遮罩（仅遮罩展开区域）       | `boolean`                                          | `true`           |
| `anchorMode`         | `anchor-mode`          | 是否启用锚点定位            | `"cursor" \| "custom" \| "disabled" \| "element"`  | `'disabled'`     |
| `anchorOffset`       | `anchor-offset`        | 锚点偏移量               | `{ x?: number; y?: number; }`                      | `{ x: 0, y: 0 }` |
