 ...
 */
import type { ApiPlugin, ApiMethodConfig } from '../types'
...
export function createOfflineCachePlugin(options: OfflineCachePluginOptions = {}): ApiPlugin {
  ...
  function shouldApply(methodName: string) {
    ...
    if (cfg.include.length > 0 && !cfg.include.includes(methodName)) return false
    if (cfg.exclude.length > 0 && cfg.exclude.includes(methodName)) return false
    return true
  }

  type Stored = { data: unknown; t: number; exp: number }
  ...
  function lsGet(key: string): Stored | null {
    try { const s = localStorage.getItem(`ldesign_offline_${key}`); return s ? JSON.parse(s) : null } catch { return null }
  }
  function lsSet(key: string, v: Stored): void {
    try { localStorage.setItem(`ldesign_offline_${key}`, JSON.stringify(v)) } catch {}
  }
  function lsDel(key: string): void { try { localStorage.removeItem(`ldesign_offline_${key}`) } catch {} }

  const isOnline = () => typeof navigator === 'undefined' ? true : navigator.onLine
  const now = () => Date.now()

  async function readCache(key: string): Promise<Stored | null> {
    if (idb) return await idbGet(key)
    return lsGet(key)
  }
  async function writeCache(key: string, data: unknown): Promise<void> {
    ...
    if (idb) return await idbSet(key, v)
    return lsSet(key, v)
  }
  async function deleteCache(key: string): Promise<void> {
    if (idb) return await idbDelete(key)
    return lsDel(key)
  }
  ...
  return {
    ...
    install(engine) {
      ...
      const resMw = async (response: any, ctx: any) => {
        ...
        if (!shouldApply(methodName)) return response
        try {
      ...
      const errMw = async (err: any, ctx: any) => {
        const methodName = ctx.methodName
        if (!shouldApply(methodName)) return

        // 仅在网络错误时启用（默认）
        const isNetworkError = !('response' in (err || {})) || (err?.response?.status === 0)
        if (cfg.onlyOnNetworkError && !isNetworkError) return
