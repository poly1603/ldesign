...
export function useMutation<TResult = unknown, TVars = unknown>(
  ...
  const mutate = useCallback(async (variables: TVars, callOptions?: Parameters<typeof api.call>[2]) => {
    if (options.lockWhilePending && loading) return Promise.reject(new Error('Mutation is pending'))
    ...
    try {
      ...
      if (options.optimistic?.snapshot && options.optimistic?.restore) {
        ...
        rollbacks.push(() => { try { options.optimistic!.restore!(snap) } catch {} })
        ...
        rollbacks.push(() => { try { options.optimistic!.target!.set(snap) } catch {} })
      }
