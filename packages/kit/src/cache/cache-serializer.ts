...
export class CacheSerializer extends EventEmitter {
   ...
   * 批量反序列化
   */
  async deserializeMany<T = any>(dataList: (string | Buffer)[], serializerName?: string): Promise<T[]> {
   ...
   * 验证序列化器
   */
  async validateSerializer(name: string, testValue: any = { test: 'data', number: 42, array: [1, 2, 3] }): Promise<boolean> {
    try {
