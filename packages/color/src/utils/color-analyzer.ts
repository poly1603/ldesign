...
import { hexToRgb, hexToHsl, rgbToHex } from './color-converter'
...
function analyzeSaturation(saturation: number): ColorAnalysis['saturationLevel'] {
  ...
  if (saturation < 80) return 'saturated'
  return 'vivid'
}
...
function generateSuggestions(
  ...
  for (const [industry, colors] of Object.entries(INDUSTRY_COLORS)) {
    if (colors.some(c => {
      ...
      if (!industryRgb || !colorRgb) return false
      
      const distance = Math.sqrt(
        Math.pow(colorRgb.r - industryRgb.r, 2) +
        Math.pow(colorRgb.g - industryRgb.g, 2) +
        Math.pow(colorRgb.b - industryRgb.b, 2)
      )
      return distance < 50
    })) {
...
function calculateAccessibilityScore(color: string): number {
  ...
  if (maxContrast >= 7) return 100
  if (maxContrast >= 4.5) return 80
  if (maxContrast >= 3) return 60
  if (maxContrast >= 2) return 40
  return 20
}
...
function calculatePopularityScore(color: string): number {
  ...
  for (const popular of popularColors) {
    ...
    const distance = Math.sqrt(
      Math.pow(rgb.r - popRgb.r, 2) +
      Math.pow(rgb.g - popRgb.g, 2) +
      Math.pow(rgb.b - popRgb.b, 2)
    )
