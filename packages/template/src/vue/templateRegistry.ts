...
export async function registerAllTemplates() {
  ...
  for (const [path, moduleLoader] of Object.entries(templateModules)) {
    ...
    // 创建异步组件
    const component = defineAsyncComponent(moduleLoader as any)
    ...
    registerTemplate({
      ...
      config: getTemplateConfig(
        templateConfig.category || 'unknown',
        templateConfig.deviceType,
        templateConfig.id || 'unknown'
      ),
    })
  }
}
