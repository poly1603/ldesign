...
function useEventListener(...args) {
  ...
  const stopWatch = watchImmediate(
    () => {
      var _a, _b
      return [
        (_b = (_a = firstParamTargets.value) == null ? void 0 : _a.map(e => unrefElement(e))) != null
          ? _b
    ...
    promise.value = new Promise((resolve, reject) => {
      try {
        ...
        if (_target == null) {
          ...
          resolve(
            _target.toDataURL(options == null ? void 0 : options.type, options == null ? void 0 : options.quality)
          )
          ...
          imgLoaded(img)
            .then(() => {
              ...
              resolve(
                canvas.toDataURL(options == null ? void 0 : options.type, options == null ? void 0 : options.quality)
              )
            })
            .catch(reject)
...
function useStorage(key, defaults2, storage, options = {}) {
  ...
  if (window2 && listenToStorageChanges) {
    tryOnMounted(() => {
      if (storage instanceof Storage) useEventListener(window2, 'storage', update, { passive: true })
...
function useCssVar(prop, target, options = {}) {
  ...
  const elRef = computed(() => {
    var _a
    return (
      unrefElement(target) || ((_a = window2 == null ? void 0 : window2.document) == null ? void 0 : _a.documentElement)
    )
  })
...
function useCycleList(list, options) {
  ...
  function getInitialValue() {
    var _a, _b
    return (_b = toValue((_a = options == null ? void 0 : options.initialValue) != null ? _a : toValue(list)[0])) !=
      null
      ? _b
      : void 0
  }
...
function useElementVisibility(element, options = {}) {
  const { window: window2 = defaultWindow, scrollTarget, threshold = 0, rootMargin, once = false } = options
...
function usePointerLock(target, options = {}) {
  ...
  async function lock(e) {
    ...
    targetElement =
      e instanceof Event ? ((_a = unrefElement(target)) != null ? _a : triggerElement.value) : unrefElement(e)
...
function useTitle(newTitle = null, options = {}) {
  ...
  const title = toRef2(
    (_b = newTitle != null ? newTitle : document2 == null ? void 0 : document2.title) != null ? _b : null
  )
  ...
  watch(
    title,
    (newValue, oldValue) => {
      if (newValue !== oldValue && document2) document2.title = format(newValue != null ? newValue : '')
    },
    { immediate: true }
  )
...
//# sourceMappingURL=chunk-AV6JYKIW.js.map
